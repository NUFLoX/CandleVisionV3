...
logger = logging.getLogger("OrderFlow.WS")
...
class MarketStream:
    def __init__(
        ...
        wall_persistence_seconds: int = 12,
        ...
        while True:
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    heartbeat_task = asyncio.create_task(self._heartbeat_loop(ws), name="orderflow_ws_heartbeat")
                    retry_delay = 3
    ...
    async def _heartbeat_loop(self, ws) -> None:
        while True:
            ...
            try:
                ...
                logger.debug("WS heartbeat ping sent")
            except Exception:
                logger.debug("WS heartbeat failed", exc_info=True)
                raise
    ...
    def _handle_message(self, raw: str) -> None:
        ...
        if op in {"ping", "pong"}:
            logger.debug("WS heartbeat ack: %s", payload)
            return
        if payload.get("success") is True and payload.get("op") == "subscribe":
            logger.debug("WS subscribed: %s", payload)
            return
    ...
    def _handle_orderbook(self, payload: dict) -> None:
        ...
        if bids:
            ...
            threshold = max(sum(bid_sizes) / max(len(bid_sizes), 1) * 2.8, 1.0)
        ...
        if asks:
            ...
            threshold = max(sum(ask_sizes) / max(len(ask_sizes), 1) * 2.8, 1.0)
