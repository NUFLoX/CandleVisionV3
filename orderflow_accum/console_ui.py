...
class ConsoleUI:
    ...
    def print_banner(self, version: str, quote: str, testnet: bool, signals_only: bool, signal_mode: str | None = None) -> None:
        ...
        if signal_mode:
            body.add_row("Signal mode", f"[bold cyan]{signal_mode}[/bold cyan]")
        panel = Panel(body, title=title, border_style="bright_blue", box=HEAVY, padding=(1, 2))
        with self._lock:
            self.console.print()
            self.console.print(panel)
            self.console.print()

    def update_session(self, *, ws_status: str | None = None, macro: int | None = None, orderflow: int | None = None) -> None:
        if ws_status is not None:
            self._stats["ws"] = ws_status
        if macro is not None:
            self._stats["macro"] = macro
        if orderflow is not None:
            self._stats["orderflow"] = orderflow

    def print_session(self, realtime_symbols: int, macro_symbols: int) -> None:
        uptime = datetime.now() - self._started_at
        grid = Table.grid(padding=(0, 2))
        grid.add_row("Uptime", str(timedelta(seconds=int(uptime.total_seconds()))))
        grid.add_row("WS", f"[bold cyan]{self._stats['ws']}[/bold cyan]")
        grid.add_row("Realtime/Macro", f"{realtime_symbols} / {macro_symbols}")
        grid.add_row("Macro/OrderFlow", f"{self._stats['macro']} / {self._stats['orderflow']}")
        with self._lock:
            self.console.print(Panel(grid, title="[bold]Session[/bold]", border_style="blue", box=ROUNDED))
    ...
    def print_signal(self, signal: Signal) -> None:
        ...
        reasons = Text(", ".join(signal.reasons[:7]) or "-", style="white")
        meta_text = ", ".join(f"{k}={v}" for k, v in list(signal.meta.items())[:7]) or "-"
        ...
        with self._lock:
            self.console.print(Panel(body, title="[bold]Signal[/bold]", border_style=border, box=HEAVY, padding=(0, 1)))
            self.console.print()
