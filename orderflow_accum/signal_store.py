...
class SignalStore:
    ...
    def promote_signal(self, *, signal_key: str, to_status: str, score_last: float | None = None) -> bool:
        cur = self.conn.cursor()
        cur.execute("SELECT symbol, timeframe, status, score_last FROM signals WHERE signal_key=?", (signal_key,))
        row = cur.fetchone()
        if row is None:
            return False
        ...
        cur.execute("UPDATE signals SET status=?, score_last=?, last_seen=? WHERE signal_key=?", (to_status, score, _utc_now(), signal_key))
        self.conn.commit()
        self.add_event(
            signal_key=signal_key,
